...
you are going to receive the energy loss from chopping wood. The program should end If the energy reaches 0 or less. 
...
$waterPerDayPerson = floatval(readline());
$foodPerDayPerson = floatval(readline());

$groupWater = $waterPerDayPerson * $countPlayers;
$groupFood = $foodPerDayPerson * $countPlayers;
...
for ($day = 1; $day <= $days; $day++) {
    ...
    if ($day % 3 === 0) {
        $groupFood -= ($groupFood / $countPlayers);
        $groupEnergy *= 1.1;
    }
}
...
if ($groupEnergy > 0) {
    ...
    printf("You will run out of energy. You will be left with {food} food and %.2f water.", $groupWater);
}